...
export async function getMeetingAreas(province_id?: string): Promise<MeetingArea[]> {
  let query = adminClient()
    .from('meeting_areas')
    .select('id, naam, active, polygon, center_lat, center_lng, radius_m, created_at')
    .order('naam')
...
export async function createMeetingArea(naam: string, polygon: [number, number][], province_id?: string | null): Promise<MeetingArea> {
  ...
  const { data, error } = await adminClient()
    .from('meeting_areas')
    .insert({ naam, polygon, center_lat, center_lng, radius_m: 200, ...(province_id ? { province_id } : {}) })
    .select('id, naam, active, polygon, center_lat, center_lng, radius_m, created_at')
    .single()

  if (error) throw new Error(error.message)
  revalidatePath('/meeting-areas')
  return data as MeetingArea
}

export async function updateMeetingArea(id: string, naam: string, polygon: [number, number][], active: boolean) {
  const lngs = polygon.map(p => p[0])
  const lats = polygon.map(p => p[1])
  const center_lng = (Math.min(...lngs) + Math.max(...lngs)) / 2
  const center_lat = (Math.min(...lats) + Math.max(...lats)) / 2
  ...
  const { error } = await adminClient()
    .from('meeting_areas')
    .update({ naam, polygon, center_lat, center_lng, active })
    .eq('id', id)
